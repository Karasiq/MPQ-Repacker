...
UnicodeString TBinaryFile::ReadString()
{
	...
	fs->Read(buffer, size);
	const UnicodeString Result = UnicodeString(buffer);
	DEL_ARRAY(buffer);
	return Result;
}
...
{
	for(unsigned i = 0; i != nMaxRead && i < fs->Size; i++)
	{
		char c = 0;
		fs->Read(&c, 1);
		if(c == 0) break;
		else if(szDest != NULL) szDest[i] = c;
	}
}
...
__fastcall TBinaryFile::SetPos(const unsigned __int64 __pos)
{
    fs->Position = __pos;
}
